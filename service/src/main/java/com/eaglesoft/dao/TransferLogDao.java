...
public class TransferLogDao {
    ...
    public Integer save(TransferLog transferLog){
        String sql = "insert into transfer_log(id,create_time,msg,bbh,data,info_id) values(?,?,?,?,?,?)";
        int i = jdbcTemplate.update(sql,
                transferLog.getId(),transferLog.getCreateTime(),transferLog.getMsg(),transferLog.getBbh(),transferLog.getData(),transferLog.getInfoId());
        return i;
    }
     ...
     * @return
     */
    public TransferLog queryLatest(Long infoId){
        String sql = "select id,create_time,msg,bbh,data from transfer_log where info_id = " + infoId + " order by create_time desc";
